...
# The access token is isolated without hiding the selected Docker context.
...
for app in server processor; do
  ...
  docker buildx build --builder "$docker_context" --platform linux/amd64 --push -f "$LAB_ROOT/Dockerfile.$app" -t "$repo/app:$tag" "$LAB_ROOT"
