...
describe("shapeAnswer", () => {
  ...
  it("resolves citations through chunkInfo, which is the shape the API really returns", () => {
    const result = shapeAnswer({
      answerText: "Recursive splitting scored 41.8 points.",
      citations: [{ sources: [{ referenceId: "0" }] }],
      references: [
        {
          chunkInfo: {
            content: "…scored 41.8 points…",
            relevanceScore: 0.77,
            documentMetadata: { uri: "gs://demo/corpus/1AbCdEf.md", title: "Chunking strategies" },
          },
        },
      ],
    });

    expect(result.citedUris).toEqual(["gs://demo/corpus/1AbCdEf.md"]);
    expect(result.chunks[0]?.relevanceScore).toBe(0.77);
  });
});
