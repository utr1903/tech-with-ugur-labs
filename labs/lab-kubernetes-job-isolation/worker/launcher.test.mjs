...
function execute(f, message, configuration = { id: f.id, message }) {
  const child = spawn(process.execPath, [launcher], {
    env: {
      ...
      EXECUTION_CONFIG: JSON.stringify(configuration),
    },
  ...
  const done = new Promise((resolve) =>
    child.on("close", (code, signal) => {
      clearTimeout(timer);
      resolve({ code, signal, error, logs });
    }),
  );
  return { child, done };
}
  ...
  for (let i = 0; i < 50; i++) {
    try {
      ...
      if (
        ...
        /^State:\s+Z/m.test(await readFile(`/proc/${pid}/status`, "utf8"))
      )
        return;
    } catch (error) {
...
test("logs command operation context separately from captured result bytes", async (t) => {
  const f = await fixture(t);
  const command = "printf 'result-only\\n'; exit 7";
  const outcome = await execute(f, command).done;
  assert.equal(outcome.code, 0, outcome.error);
  const records = outcome.logs
    .trim()
    .split("\n")
    .filter(Boolean)
    .map((line) => JSON.parse(line));
  const execution = records.filter(
    (entry) => entry.operation === "Running shell command",
  );
  assert.deepEqual(
    execution.map((entry) => entry.level),
    ["info", "info"],
  );
  assert.equal(execution[0].command, "/bin/sh");
  assert.deepEqual(execution[0].args, ["-c", command]);
  assert.equal(execution[1].exitCode, 7);
  assert.equal(
    await readFile(join(f.data, `${f.id}.md`), "utf8"),
    "result-only\n",
  );
  assert.ok(records.every((entry) => entry.output === undefined));
});

test("logs collection failure with command context and no raw private result bytes", async (t) => {
  const f = await fixture(t);
  await symlink(join(f.home, "private"), join(f.data, `${f.id}.md`));
  const outcome = await execute(f, "true").done;
  ...
  const records = outcome.logs
    ...
    .filter(Boolean)
    .map((line) => JSON.parse(line));
  const collection = records.filter(
    (entry) => entry.operation === "Collecting execution results",
  );
  assert.deepEqual(
    collection.map((entry) => entry.level),
    ["info", "error"],
  );
  assert.equal(collection[1].id, f.id);
  assert.deepEqual(collection[1].args, ["-c", "true"]);
  assert.equal(typeof collection[1].err.category, "string");
});
...
test("caps endless output and stops its entire group promptly", async (t) => {
  ...
  const actual = await result(
    f,
    'sleep 100 & echo $! > "$HOME/child.pid"; yes x',
  );
  ...
  await assertStopped(
    Number(await readFile(join(f.home, "child.pid"), "utf8")),
  );
});
...
test("does not mark an exactly full completed output as truncated", async (t) => {
  ...
  const actual = await result(
    f,
    "node -e 'process.stdout.write(Buffer.alloc(65536, 97))'",
  );
  assert.equal(actual.output.length, 65536);
  assert.deepEqual(actual.metadata, { exitCode: 0, truncated: false });
});
...
test("preserves normal exit while cleaning background descendants", async (t) => {
  ...
  const actual = await result(
    f,
    'sleep 100 & echo $! > "$HOME/child.pid"; printf done; exit 7',
  );
...
test("escalates cleanup for descendants ignoring SIGTERM", async (t) => {
  ...
  const actual = await result(
    f,
    'sh -c \'trap "" TERM; echo $$ > "$HOME/child.pid"; while :; do sleep 1; done\' & sleep 0.1; printf done',
  );
  assert.equal(actual.output.toString(), "done");
  await assertStopped(
    Number(await readFile(join(f.home, "child.pid"), "utf8")),
...
test("does not wait indefinitely for pipes retained by a new session", async (t) => {
  ...
  const running = execute(
    f,
    "setsid sh -c 'echo $$ > \"$HOME/escaped.pid\"; sleep 100' & sleep 0.1; printf done",
  );
...
test("applies one byte budget across stdout and stderr", async (t) => {
  ...
  const actual = await result(
    f,
    "node -e 'process.stdout.write(Buffer.alloc(40000,97)); process.stderr.write(Buffer.alloc(40000,98))'",
  );
  assert.equal(actual.output.length, 65536);
  assert.equal(actual.metadata.truncated, true);
});
...
test("launcher termination stops waiting command without successful metadata", async (t) => {
  ...
  assert.notEqual(outcome.code, 0);
  const stopping = outcome.logs
    .trim()
    .split("\n")
    .filter(Boolean)
    .map((line) => JSON.parse(line))
    .filter((entry) => entry.operation === "Stopping worker");
  assert.deepEqual(
    stopping.map((entry) => entry.level),
    ["info", "info"],
  );
  assert.equal(stopping[1].signal, "SIGTERM");
  const lifecycle = outcome.logs
    .trim()
    .split("\n")
    .filter(Boolean)
    .map((line) => JSON.parse(line))
    .filter((entry) => entry.operation === "Running worker");
  assert.deepEqual(
    lifecycle.map((entry) => entry.level),
    ["info", "error"],
  );
...
test("rejects malformed UUID and configuration before producing files", async (t) => {
  for (const configuration of [
    null,
    { id: "../escape", message: "true" },
    { id: `${randomUUID()}\n`, message: "true" },
    { id: randomUUID(), message: "true", extra: 1 },
    { id: randomUUID(), message: 1 },
  ]) {
    const f = await fixture(t);
