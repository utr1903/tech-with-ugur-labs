...
test('serves exact harmless script bytes and records path/source without executing it', async () => {
  const child = spawn(process.execPath, ['fixture/server.mjs'], {env:{...process.env,PORT:'0'},stdio:['ignore','pipe','pipe']});
  let logs = '';
  child.stdout.on('data', data => {logs += data;});
  try {
    for(let i=0;i<100 && !logs.includes('listening');i++) await new Promise(resolve=>setTimeout(resolve,20));
    const entry = logs.trim().split('\n').filter(Boolean).map(line=>JSON.parse(line)).find(line=>line.event==='listening');
    ...
    assert.equal(await response.text(), "printf 'SIMULATED_DOWNLOAD_MARKER\\n'\n");
    const absent = await fetch(`http://127.0.0.1:${entry.port}/health`);
    assert.equal(absent.status,404);
    await new Promise(resolve=>setTimeout(resolve,20));
    const request = logs.trim().split('\n').filter(Boolean).map(line=>JSON.parse(line)).find(line=>line.path==='/marker.sh');
